...
def classify_margin_trend(margin_series: pd.Series) -> dict:
    ...
    if change > 2:
        ...
        result["score"] = 35
        result["message"] = "Die Nettomarge ist zuletzt spürbar gefallen."
    else:
        result["status"] = "➡️ Margen weitgehend stabil"
        result["score"] = 65
        result["message"] = "Die Nettomarge zeigt zuletzt keine starke Veränderung."
...
def classify_fcf_dynamics(fcf_series: pd.Series) -> dict:
    ...
    if latest > 0 and previous <= 0:
        ...
        result["score"] = 90
        result["message"] = "Der Free Cashflow ist zuletzt in den positiven Bereich gedreht."
    elif latest > 0 and previous > 0 and latest > previous:
        result["status"] = "🟢 FCF verbessert sich"
        result["score"] = 85
        result["message"] = "Der positive Free Cashflow hat sich gegenüber dem Vorjahr verbessert."
    elif latest > 0 and previous > 0 and latest < previous:
        result["status"] = "🟠 FCF schwächer"
        result["score"] = 50
        result["message"] = "Der Free Cashflow bleibt positiv, ist aber gegenüber dem Vorjahr gefallen."
    elif latest < 0 and previous < 0 and latest < previous:
        result["status"] = "🔴 FCF-Belastung steigt"
        result["score"] = 20
        result["message"] = "Der negative Free Cashflow ist zuletzt noch negativer geworden."
    else:
        result["status"] = "➡️ FCF uneinheitlich"
        result["score"] = 45
        result["message"] = "Die FCF-Entwicklung lässt sich derzeit nicht eindeutig als Trend einordnen."
